  ...
  create(context) {
    ...
    return {
      ImportDeclaration(node) {
        ...
        const deepFilePattern = deepFilePatterns.find((pattern) => micromatch.isMatch(importTo, pattern));
        const deepFileParts = deepFilePattern ? deepFilePattern.split('/') : [];
        const deepFileLevel = deepFileParts.length || 2;

        console.log(segments.length, deepFileLevel);
        ...
        const getFixedImport = (deepLevel) => {
          ...
          for (let i = 0; i < deepLevel - 2; i++) {
            ...
            resImport += resImport.slice(-1) + `/${lastPart}'`;
          }
